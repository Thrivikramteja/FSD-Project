...
// Assuming you have imported or applied the styles from care_job.css
...
        description: '',
        location: '',
        pay: '', // Remains as string for controlled input
        type: '',
        startDate: '',
        endDate: ''
    });
...
        const dataToSend = {
            ...formData,
            // Convert pay to a number if it's not empty, otherwise keep null for API
...
            });
            ...
            // Try to parse the response body regardless of the status, 
            // as the server might send a JSON error object.
...
                
            } else {
                // If response is not OK (e.g., 400 or 500 status), use the message from the result, or fall back.
...
            }
        } catch (err) {
            // This catches network errors (e.g., CORS, offline) or JSON parsing errors.
            console.error("Fetch error:", err);
            // Display a more informative error for the user
...
    // JSX structure (Using simple label-input placement to avoid CSS conflicts)
    return (
