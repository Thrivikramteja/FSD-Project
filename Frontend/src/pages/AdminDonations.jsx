...
const AdminDonations = () => {
    ...
    useEffect(() => {
        fetch('http://localhost:3000/api/admin/donations-analytics', { credentials: 'include' })
            .then(res => {
    ...
    const handleAudit = async (home) => {
        setAuditHome(home);
        try {
            const res = await fetch(`http://localhost:3000/api/admin/carehome-donors/${home.carehomeId}`, { credentials: 'include' });
            if (!res.ok) throw new Error(`API Error: ${res.status}`);
