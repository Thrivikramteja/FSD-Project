...
const DonorControl = () => {
    ...
    useEffect(() => {
        fetch('http://localhost:3000/api/admin/all-donors', { credentials: 'include' })
            .then(res => res.json()).then(data => setDonors(data.donors || []));
    }, []);

    const handleAudit = async (user) => {
        setLoading(true);
        const res = await fetch(`http://localhost:3000/api/admin/donor-stats/${user.userId}`, { credentials: 'include' });
