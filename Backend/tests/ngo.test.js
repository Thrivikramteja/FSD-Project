...
// ════════════════════════════════════════════════════════════════════════════
// 2. GET ALL NGOs  (public)
// ════════════════════════════════════════════════════════════════════════════

describe("GET /api/ngos", () => {
  test("returns empty array when no NGOs exist", async () => {
    const res = await request(app).get("/api/ngos");
    expect(res.status).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
    expect(res.body.length).toBe(0);
  });

  test("returns enriched NGO objects with stats fields", async () => {
    await createNGO();
    const res = await request(app).get("/api/ngos");
    expect(res.status).toBe(200);
    expect(res.body.length).toBeGreaterThan(0);
    expect(res.body[0]).toHaveProperty("totalFundsRaised");
    expect(res.body[0]).toHaveProperty("totalRegistrations");
    expect(res.body[0]).toHaveProperty("fundraisersCreated");
    expect(res.body[0]).toHaveProperty("careHomesBenefited");
  });
});
...
describe("GET /api/events", () => {
  test("returns only upcoming events", async () => {
    ...
    expect(res.status).toBe(200);
    const names = res.body.map((e) => e.event_name);
